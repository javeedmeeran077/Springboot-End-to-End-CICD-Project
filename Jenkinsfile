pipeline {
    ...
    stages {
        ...
        stage('Static Code Analysis') {
            ...
            environment {
                SONAR_URL = "http://54.236.48.155:9000"
            }
        ...
        stage('Upload Code Artifacts') {
            ...
            steps {
                script {
                    ...
                    if (params.YES) {
                        sh 'jfrog rt upload --url http://52.87.247.76:8082/artifactory/ --access-token ${ARTIFACTORY_ACCESS_TOKEN} target/*.jar springboot-web-app/'
                    } else {
        ...
        stage('Build & Push Docker Image') {
            ...
            environment {
                DOCKER_IMAGE = "javeed1998/spring-docker:${BUILD_NUMBER}"
        ...
        stage('Updating Deployment File') {
            ...
            environment {
                ...
                GIT_USER_NAME = "javeedmeeran077"
            }
            steps {
                // Replacing the previous BUILD_NUMBER with NEW_BUILD_NUMBER and pushing the changes to Github
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                    sh '''
                        git config user.email "javeedmeeran1@yahoo.com"
                        git config user.name "javeedmeeran077"
